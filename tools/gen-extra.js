const packages = require('../src/packages/extra/packages.json');
const fs = require('fs');
...
packages.forEach(pkg => {
	if (pkg !== 'undefined') {
		...
		const pkgScriptsFolder = `src/packages/extra/${pkg.name}`;
...
fs.writeFileSync(`src/packages/extra/packages.json`, JSON.stringify(updatedPkgs, null, '\t'), 'utf-8');
fs.writeFileSync(`src/packages/extra/wget-list`, updatedPkgs.map(x => `${x.url}`).join('\n'), 'utf-8');
fs.writeFileSync(`src/packages/extra/md5sums`, String(updatedPkgs.map(x => `${x.md5} ${x.fileName}`).join('\n')), 'utf-8');
fs.writeFileSync(`src/packages/extra/pkg-list`, String(updatedPkgs.map(x => x.fileName).join('\n')), 'utf-8');